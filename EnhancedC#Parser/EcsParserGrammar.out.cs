...
namespace Loyc.Ecs.Parser
{
	...
	partial class EcsParser
	{
		...
		LNode ComplexId()
		{
			...
			// Line 166: (TT.Dot IdAtom (TParams)?)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Dot) {
					switch (LA(1)) {
		...
		LNode IdAtom()
		{
			// line 174
			LNode r;
			// Line 175: ( TT.Substitute Atom | TT.Operator AnyOperator | (TT.Id|TT.TypeKeyword) | IdNotLinqKeyword )
			switch (LA0) {
			case TT.Substitute:
				{
					var t = MatchAny();
		...
		void TParams(ref LNode r)
		{
			...
			Token end;
			// Line 200: ( TT.LT (DataType (TT.Comma DataType)*)? TT.GT | TT.Dot TT.LBrack TT.RBrack | TT.Not TT.LParen TT.RParen )
			la0 = LA0;
			if (la0 == TT.LT) {
				Skip();
				// Line 200: (DataType (TT.Comma DataType)*)?
				switch (LA0) {
				case TT.ContextualKeyword:
				case TT.Id:
				case TT.Operator:
		...
		bool TypeSuffixOpt(bool afterAsOrIs, out Token? dimensionBrack, ref LNode e)
		{
				...
				if (la0 == TT.QuestionMark) {
					...
					if ((count = CountDims(LT(0), true)) > 0) {
						...
						if (la1 == TT.RBrack) {
							...
							// Line 260: greedy(&{(count = CountDims(LT($LI), @false)) > 0} TT.LBrack TT.RBrack)*
							for (;;) {
								la0 = LA0;
								if (la0 == TT.LBrack) {
			...
			match1:
				{
					var t = MatchAny();
					// Line 248: (&!{afterAsOrIs} | &!(((TT.Add|TT.AndBits|TT.At|TT.Forward|TT.Id|TT.IncDec|TT.LBrace|TT.Literal|TT.LParen|TT.Mul|TT.New|TT.Not|TT.NotBits|TT.Sub|TT.Substitute|TT.TypeKeyword) | IdNotLinqKeyword)))
					if (!afterAsOrIs) {
					} else
		...
		LNode ComplexNameDecl(bool thisAllowed, out bool hasThis)
		{
			...
			// Line 281: (ComplexThisDecl | IdAtom (TT.ColonColon IdAtom)? (TParamsDecl)? (TT.Dot IdAtom (TParamsDecl)?)* (TT.Dot ComplexThisDecl)?)
			la0 = LA0;
			if (la0 == TT.This) {
				...
				// Line 288: (TT.Dot IdAtom (TParamsDecl)?)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Dot) {
						switch (LA(1)) {
			...
			stop:;
				// Line 292: (TT.Dot ComplexThisDecl)?
				la0 = LA0;
				if (la0 == TT.Dot) {
					la1 = LA(1);
					if (la1 == TT.This) {
		...
		void TParamsDecl(ref LNode r)
		{
			...
			VList<LNode> list = new VList<LNode>(r);
			Token end;
			// Line 309: ( TT.LT (TParamDecl (TT.Comma TParamDecl)*)? TT.GT | TT.Dot TT.LBrack TT.RBrack | TT.Not TT.LParen TT.RParen )
			la0 = LA0;
			if (la0 == TT.LT) {
				Skip();
				// Line 309: (TParamDecl (TT.Comma TParamDecl)*)?
				switch (LA0) {
				case TT.AttrKeyword:
				case TT.ContextualKeyword:
				case TT.Id:
		...
		LNode TParamDecl()
		{
			...
			// Line 321: (ComplexId / NormalAttributes TParamAttributeKeywords IdAtom)
			switch (LA0) {
			case TT.ContextualKeyword:
			case TT.Id:
			case TT.Operator:
		...
		LNode Atom()
		{
			...
			default:
				{
					// Line 425: greedy(~(EOF|TT.Comma|TT.Semicolon))*
					for (;;) {
						la0 = LA0;
		...
		LNode PrimaryExpr()
		{
			...
			// Line 543: (TT.NullDot PrimaryExpr)?
			la0 = LA0;
			if (la0 == TT.NullDot) {
				Skip();
				var rhs = PrimaryExpr();
		...
		{
			TokenType la1;
			// Line 550: greedy( (TT.ColonColon|TT.Dot|TT.PtrArrow|TT.QuickBind) Atom / PrimaryExpr_NewStyleCast / TT.LParen TT.RParen | TT.LBrack TT.RBrack | TT.QuestionMark TT.LBrack TT.RBrack | TT.IncDec | &(TParams ~(TT.ContextualKeyword|TT.Id)) ((TT.LT|TT.Not) | TT.Dot TT.LBrack) => TParams | BracedBlockOrTokenLiteral )*
			for (;;) {
				switch (LA0) {
				case TT.Dot:
					{
		...
		LNode Expr(Precedence context)
		{
			...
			match3:
				{
					// Line 684: (TT.LT TT.LT Expr | TT.GT TT.GT Expr)
					la0 = LA0;
					if (la0 == TT.LT) {
						Skip();
						Match((int) TT.LT);
		...
		public LNode ExprStart(bool allowUnassignedVarDecl)
		{
			TokenType la0, la1;
			LNode result = default(LNode);
			// Line 702: ((TT.Id | IdNotLinqKeyword) TT.Colon ExprStart2 / ExprStart2)
			do {
				la0 = LA0;
				if (la0 == TT.Id) {
					la1 = LA(1);
			...
			match1:
				{
					...
					// Line 703: (TT.Id | IdNotLinqKeyword)
					la0 = LA0;
					if (la0 == TT.Id)
						argName = MatchAny();
					else
		...
		LNode VarDeclExpr(out bool hasInitializer, VList < LNode > attrs)
		{
			...
			// Line 839: (RestOfPropertyDefinition / VarInitializerOpt)
			switch (LA0) {
			case TT.At:
			case TT.ContextualKeyword:
			case TT.Forward:
		...
		{
			TokenType la0, la1;
			// Line 885: (EOF / ExprStart nongreedy(TT.Comma ExprStart)* (TT.Comma)? EOF)
			do {
				la0 = LA0;
				if (la0 == EOF) {
					la1 = LA(1);
			...
			match2:
				{
					...
					// Line 890: nongreedy(TT.Comma ExprStart)*
					for (;;) {
						la0 = LA0;
						if (la0 == TT.Comma) {
							la1 = LA(1);
		...
		LNode BracedBlockOrTokenLiteral(Symbol spaceName = null, Symbol target = null, int startIndex = -1)
		{
			TokenType la0;
			LNode result = default(LNode);
			// Line 897: (BracedBlock | TokenLiteral)
			la0 = LA0;
			if (la0 == TT.LBrace)
		...
		{
			TokenType la0, la1;
			// Line 919: (&!{Down($LI) && Up(Try_Scan_AsmOrModLabel(0))} TT.LBrack TT.RBrack)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.LBrack) {
		...
		void AttributeContents(ref VList<LNode> attrs)
		{
			...
			// Line 932: greedy((TT.ContextualKeyword|TT.Id|TT.Return) TT.Colon)?
			la0 = LA0;
		...
		LNode MethodOrPropertyOrVarStmt(int startIndex, VList<LNode> attrs)
		{
			TokenType la0;
			LNode result = default(LNode);
			// Line 1372: ( TraitDecl / AliasDecl / MethodOrPropertyOrVar )
			do {
				la0 = LA0;
		...
		LNode KeywordStmt(int startIndex, VList<LNode> attrs, bool hasWordAttrs)
		{
			...
			// Line 1385: ( ((IfStmt | EventDecl | DelegateDecl | SpaceDecl | EnumDecl | CheckedOrUncheckedStmt | DoStmt | CaseStmt) | (GotoCaseStmt / GotoStmt) | ReturnBreakContinueThrow | WhileStmt | ForStmt | ForEachStmt | SwitchStmt) | (UsingStmt / UsingDirective) | LockStmt | FixedStmt | TryStmt )
			switch (LA0) {
			case TT.If:
				{
					r = IfStmt(startIndex);
		...
		LNode IdStmt(int startIndex, VList<LNode> attrs, bool hasWordAttrs)
		{
			...
			bool addAttrs = false;
			string showWordAttrErrorFor = null;
			// Line 1429: ( Constructor / BlockCallStmt / LabelStmt / &(DataType TT.This) DataType => MethodOrPropertyOrVar / ExprStatement )
			do {
				switch (LA0) {
				case TT.ContextualKeyword:
				case TT.Id:
		...
		LNode OtherStmt(int startIndex, VList<LNode> attrs, bool hasWordAttrs)
		{
			...
			bool addAttrs = false;
			string showWordAttrErrorFor = null;
			// Line 1465: ( BracedBlock / &(TT.NotBits (TT.ContextualKeyword|TT.Id|TT.This) TT.LParen TT.RParen TT.LBrace TT.RBrace) Destructor / TT.Semicolon / LabelStmt / default ExprStatement / AssemblyOrModuleAttribute / OperatorCastMethod )
			do {
				switch (LA0) {
				case TT.LBrace:
					{
		...
		LNode ExprStatement()
		{
			...
			// Line 1499: ((EOF|TT.Catch|TT.Else|TT.Finally|TT.While) =>  | TT.Semicolon)
			switch (LA0) {
			case EOF:
			case TT.Catch:
			case TT.Else:
		...
		{
			TokenType la0;
			// Line 1509: greedy(~(EOF|TT.LBrace|TT.Semicolon))*
			for (;;) {
				la0 = LA0;
			...
			// Line 1510: greedy(TT.Semicolon | TT.LBrace (TT.RBrace)?)?
			la0 = LA0;
			if (la0 == TT.Semicolon)
				Skip();
			else if (la0 == TT.LBrace) {
		...
		LNode RestOfSpaceDecl(int startIndex, Symbol kind)
		{
			...
			// Line 1534: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
				// line 1535
		...
		LNode UsingDirective(int startIndex, VList<LNode> attrs)
		{
			...
			// Line 1552: (&{Is($LI, S.Static)} TT.AttrKeyword ExprStart TT.Semicolon / ExprStart (&{nsName.Calls(S.Assign, 2)} RestOfAlias / TT.Semicolon))
			do {
				la0 = LA0;
				if (la0 == TT.AttrKeyword) {
					if (Is(0, S.Static)) {
			...
			matchExprStart:
				{
					nsName = ExprStart(true);
					// Line 1557: (&{nsName.Calls(S.Assign, 2)} RestOfAlias / TT.Semicolon)
					do {
						switch (LA0) {
						case TT.Semicolon:
							{
		...
		LNode RestOfAlias(LNode oldName, int startIndex, LNode newName)
		{
			...
			// Line 1573: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
				// line 1574
		...
		LNode EnumDecl(int startIndex)
		{
			...
			var name = ComplexNameDecl();
			var bases = BaseListOpt();
			// Line 1584: (TT.Semicolon | TT.LBrace TT.RBrace)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
				// line 1585
		...
		{
			TokenType la0;
			// Line 1596: (TT.Colon DataType (TT.Comma DataType)* | )
			la0 = LA0;
			if (la0 == TT.Colon) {
				// line 1596
		...
		KeyValuePair<Symbol,LNode> WhereClause()
		{
			...
			// Line 1640: (TT.Comma WhereConstraint)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Comma) {
					Skip();
		...
		{
			TokenType la0;
			// Line 1645: ( (TT.Class|TT.Struct) | TT.New &{LT($LI).Count == 0} TT.LParen TT.RParen | DataType )
			la0 = LA0;
		...
		LNode MethodOrPropertyOrVar(int startIndex, VList<LNode> attrs)
		{
			...
			// Line 1686: ( &{!isNamedThis} VarInitializerOpt (TT.Comma ComplexNameDecl VarInitializerOpt)* TT.Semicolon / &{!isNamedThis} MethodArgListAndBody | RestOfPropertyDefinition )
			switch (LA0) {
			case TT.Comma:
			...
			case TT.Set:
				{
					...
					// Line 1691: (TT.Comma ComplexNameDecl VarInitializerOpt)*
					for (;;) {
						la0 = LA0;
						if (la0 == TT.Comma) {
							Skip();
		...
		LNode VarInitializer(bool isArray)
		{
			...
			LNode result = default(LNode);
			Skip();
			// Line 1718: (&{isArray} &{Down($LI) && Up(HasNoSemicolons())} TT.LBrace TT.RBrace / ExprStart)
			la0 = LA0;
			if (la0 == TT.LBrace) {
				if (isArray) {
		...
		LNode MethodArgListAndBody(int startIndex, VList<LNode> attrs, Symbol kind, LNode type, LNode name)
		{
			...
			// Line 1754: (TT.Colon (TT.Base|TT.This) TT.LParen TT.RParen)?
			la0 = LA0;
			if (la0 == TT.Colon) {
				Skip();
			...
			// Line 1772: (default TT.Semicolon | MethodBodyOrForward)
			do {
				switch (LA0) {
				case TT.Semicolon:
					goto match1;
		...
		LNode MethodBodyOrForward(bool isProperty, out LNode propInitializer, bool isExpression = false)
		{
			...
			// Line 1798: ( TT.Forward ExprStart SemicolonIf | TT.LambdaArrow ExprStart SemicolonIf | TokenLiteral (&{!isExpression} TT.Semicolon)? | BracedBlock greedy(&{isProperty} TT.Set ExprStart SemicolonIf)? )
			la0 = LA0;
			if (la0 == TT.Forward) {
				var op = MatchAny();
				...
				var e = TokenLiteral();
				// Line 1800: (&{!isExpression} TT.Semicolon)?
				la0 = LA0;
				if (la0 == TT.Semicolon) {
				...
				// Line 1804: greedy(&{isProperty} TT.Set ExprStart SemicolonIf)?
				la0 = LA0;
				if (la0 == TT.Set) {
		...
		{
			TokenType la0;
			// Line 1812: (&{isStatement} TT.Semicolon / )
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				if (isStatement)
					Skip();
		...
		LNode Constructor(int startIndex, VList<LNode> attrs)
		{
			...
			LNode r;
			Token n;
			// Line 1842: ( &{_spaceName == LT($LI).Value} (TT.ContextualKeyword|TT.Id) &(TT.LParen TT.RParen (TT.LBrace|TT.Semicolon)) / &{_spaceName != S.Fn || LA($LI + 3) == TT.LBrace} TT.This &(TT.LParen TT.RParen (TT.LBrace|TT.Semicolon)) / (TT.ContextualKeyword|TT.Id|TT.This) &(TT.LParen TT.RParen TT.Colon) )
			do {
				la0 = LA0;
		...
		LNode EventDecl(int startIndex)
		{
			...
			var type = DataType();
			var name = ComplexNameDecl();
			// Line 1881: (TT.Comma ComplexNameDecl (TT.Comma ComplexNameDecl)*)?
			la0 = LA0;
			if (la0 == TT.Comma) {
				// line 1881
				...
				// Line 1882: (TT.Comma ComplexNameDecl)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
			...
			// Line 1885: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				lit_semi = MatchAny();
				// line 1886
		...
		LNode BlockCallStmt(int startIndex)
		{
			...
			LNode block;
			// Line 1926: ( TT.LParen TT.RParen (BracedBlock | TT.Id => Stmt) | TT.Forward ExprStart TT.Semicolon | BracedBlock )
			la0 = LA0;
			if (la0 == TT.LParen) {
				var lp = MatchAny();
				var rp = Match((int) TT.RParen);
				...
				// Line 1927: (BracedBlock | TT.Id => Stmt)
				la0 = LA0;
				if (la0 == TT.LBrace)
					block = BracedBlock();
				else {
		...
		LNode TryStmt(int startIndex)
		{
			...
			LNode varExpr;
			LNode whenExpr;
			// Line 2097: greedy(TT.Catch (TT.LParen TT.RParen / ) (&{Is($LI, _when)} TT.ContextualKeyword TT.LParen TT.RParen / ) Stmt)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Catch) {
					var kw = MatchAny();
					// Line 2098: (TT.LParen TT.RParen / )
					la0 = LA0;
					if (la0 == TT.LParen) {
						la1 = LA(1);
						if (la1 == TT.RParen) {
						...
						// line 2099
						varExpr = MissingHere();
					// Line 2100: (&{Is($LI, _when)} TT.ContextualKeyword TT.LParen TT.RParen / )
					la0 = LA0;
					if (la0 == TT.ContextualKeyword) {
						if (Is(0, _when)) {
							la1 = LA(1);
			...
			// Line 2107: greedy(TT.Finally Stmt)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Finally) {
					var kw = MatchAny();
		...
		LNode ExprOrNull(bool allowUnassignedVarDecl = false)
		{
			TokenType la0;
			LNode result = default(LNode);
			// Line 2124: ((EOF|TT.Comma|TT.Semicolon) =>  / ExprStart)
			la0 = LA0;
		...
		{
			TokenType la0, la1;
			// Line 2138: nongreedy(ExprOpt (TT.Comma &{allowTrailingComma} EOF / TT.Comma ExprOpt)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(ExprOpt(allowUnassignedVarDecl));
				// Line 2139: (TT.Comma &{allowTrailingComma} EOF / TT.Comma ExprOpt)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		{
			// Line 248: ((TT.Add|TT.AndBits|TT.At|TT.Forward|TT.Id|TT.IncDec|TT.LBrace|TT.Literal|TT.LParen|TT.Mul|TT.New|TT.Not|TT.NotBits|TT.Sub|TT.Substitute|TT.TypeKeyword) | IdNotLinqKeyword)
			switch (LA0) {
			case TT.Add:
			case TT.AndBits:
			case TT.At:
		...
		{
			// Line 615: ((TT.Add|TT.AndBits|TT.BQString|TT.Dot|TT.Mul|TT.Sub) | TT.IncDec TT.LParen)
			switch (LA0) {
			case TT.Add:
			case TT.AndBits:
			case TT.BQString:
		...
		{
			TokenType la0;
			// Line 1923: ( TT.LParen TT.RParen (TT.LBrace TT.RBrace | TT.Id) | TT.LBrace TT.RBrace | TT.Forward )
			la0 = LA0;
			if (la0 == TT.LParen) {
				if (!TryMatch((int) TT.LParen))
					return false;
				if (!TryMatch((int) TT.RParen))
					return false;
				// Line 1923: (TT.LBrace TT.RBrace | TT.Id)
				la0 = LA0;
				if (la0 == TT.LBrace) {
					if (!TryMatch((int) TT.LBrace))
						return false;
