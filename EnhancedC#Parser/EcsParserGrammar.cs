...
namespace Ecs.Parser
{
	...
	partial class EcsParser
	{
		...
		LNode IdAtom()
		{
			TokenType la0;
			LNode r;
			// Line 154: ( TT.Substitute Atom | TT.@operator AnyOperator | (TT.TypeKeyword|TT.ContextualKeyword|TT.Id) )
			la0 = LA0;
		...
		bool TypeSuffixOpt(bool afterAsOrIs, out Token? dimensionBrack, ref LNode e)
		{
			...
			bool result = false;
			dimensionBrack = null;
			// Line 225: greedy( TT.QuestionMark (&!{afterAsOrIs} | &!((TT.@new|TT.Mul|TT.Add|TT.LParen|TT.TypeKeyword|TT.SQString|TT.NotBits|TT.OtherLit|TT.Number|TT.AndBits|TT.Id|TT.Symbol|TT.ContextualKeyword|TT.Not|TT.IncDec|TT.Substitute|TT.LBrace|TT.At|TT.String|TT.Forward|TT.Sub))) | TT.Mul | &{(count = CountDims(LT($LI), @true)) > 0} TT.LBrack TT.RBrack greedy(&{(count = CountDims(LT($LI), @false)) > 0} TT.LBrack TT.RBrack)* )*
			 for (;;) {
				la0 = LA0;
				if (la0 == TT.QuestionMark) {
					if (!afterAsOrIs || !Try_TypeSuffixOpt_Test0(1) && (count = CountDims(LT(1), true)) > 0) {
						var t = MatchAny();
						// Line 225: (&!{afterAsOrIs} | &!((TT.@new|TT.Mul|TT.Add|TT.LParen|TT.TypeKeyword|TT.SQString|TT.NotBits|TT.OtherLit|TT.Number|TT.AndBits|TT.Id|TT.Symbol|TT.ContextualKeyword|TT.Not|TT.IncDec|TT.Substitute|TT.LBrace|TT.At|TT.String|TT.Forward|TT.Sub)))
						if (!afterAsOrIs) {
						} else
							Check(!Try_TypeSuffixOpt_Test0(0), "!((TT.@new|TT.Mul|TT.Add|TT.LParen|TT.TypeKeyword|TT.SQString|TT.NotBits|TT.OtherLit|TT.Number|TT.AndBits|TT.Id|TT.Symbol|TT.ContextualKeyword|TT.Not|TT.IncDec|TT.Substitute|TT.LBrace|TT.At|TT.String|TT.Forward|TT.Sub))");
		...
		LNode Atom()
		{
			TokenType la0, la1;
			LNode r;
			// Line 360: ( (TT.Substitute|TT.Dot) Atom | TT.@operator AnyOperator | (TT.TypeKeyword|TT.ContextualKeyword|@`#.`(TT, noMacro(@base))|@`#.`(TT, noMacro(@this))|TT.Id) | (TT.SQString|TT.Symbol|TT.OtherLit|TT.Number|TT.String) | ExprInParensAuto | BracedBlock | NewExpr | TT.At TT.LBrack TT.RBrack | (TT.@checked|TT.@unchecked) TT.LParen TT.RParen | (@`#.`(TT, noMacro(@default))|TT.@typeof|TT.@sizeof) TT.LParen TT.RParen | TT.@delegate TT.LParen TT.RParen TT.LBrace TT.RBrace )
			 switch (LA0) {
			case TT.Dot:
			case TT.Substitute:
				{
		...
		LNode PrimaryExpr()
		{
			TokenType la1;
			var e = Atom();
			// Line 487: greedy( (TT.QuickBind|TT.PtrArrow|TT.ColonColon|TT.Dot) Atom / PrimaryExpr_NewStyleCast / TT.LParen TT.RParen | TT.At TT.LBrace TT.RBrace | TT.LBrack TT.RBrack | TT.IncDec | &(TParams ~(TT.ContextualKeyword|TT.Id)) ((TT.LT|TT.Not) | TT.Dot TT.LBrack) => TParams | BracedBlock )*
			 for (;;) {
				switch (LA0) {
				case TT.Dot:
					{
		...
		{
			TokenType la2;
			// Line 543: ( (TT.Add|TT.Not|TT.IncDec|TT.NotBits|TT.AndBits|TT.Sub|TT.Mul|TT.Forward) PrefixExpr | (&{Down($LI) && Up(Scan_DataType() && LA0 == EOF)} TT.LParen TT.RParen &!(((TT.Add|TT.Sub) | TT.IncDec TT.LParen)) PrefixExpr / TT.Power PrefixExpr / NullDotExpr) )
			 do {
				switch (LA0) {
				case TT.Add:
				case TT.AndBits:
		...
		LNode Expr(Precedence context)
		{
			...
			Precedence prec;
			var e = PrefixExpr();
			// Line 604: greedy( &{context.CanParse(prec = InfixPrecedenceOf($LA))} (TT.OrXor|TT.Add|TT.GT|TT.XorBits|TT.Set|TT.LT|TT.NotBits|TT.And|TT.DotDot|TT.Power|TT.AndBits|TT.Sub|TT.EqNeq|TT.LEGE|TT.OrBits|TT.NullCoalesce|TT.BQString|TT.LambdaArrow|TT.DivMod|TT.@in|TT.CompoundSet|TT.Mul) Expr | &{context.CanParse(prec = InfixPrecedenceOf($LA))} (TT.@using|TT.@as|TT.@is) DataType | &{context.CanParse(EP.Shift)} &{LT($LI).EndIndex == LT($LI + 1).StartIndex} (TT.LT TT.LT Expr | TT.GT TT.GT Expr) | &{context.CanParse(EP.IfElse)} TT.QuestionMark Expr TT.Colon Expr )*
			 for (;;) {
				switch (LA0) {
				case TT.GT:
				case TT.LT:
		...
		{
			TokenType la0;
			// Line 686: (SubConditional | ~(TT.Comma|TT.QuestionMark|TT.Semicolon|EOF|TT.Colon))*
			 for (;;) {
				la0 = LA0;
				if (la0 == TT.QuestionMark)
					SubConditional();
		...
		void AttributeContents(ref RWList<LNode> attrs)
		{
			...
			// Line 778: greedy((@`#.`(TT, noMacro(@return))|TT.ContextualKeyword|TT.Id) TT.Colon)?
			la0 = LA0;
		...
		int WordAttributes(ref RWList<LNode> attrs)
		{
			...
			if (LA0 == TT.Id && ((LA1 = LA(1)) == TT.Set || LA1 == TT.LParen || LA1 == TT.Dot))
				 return 0;
			// Line 880: (TT.AttrKeyword | (TT.@new|TT.ContextualKeyword|TT.Id) &(( DataType (TT.AttrKeyword|TT.TypeKeyword|TT.ContextualKeyword|TT.Id) | (TT.AttrKeyword|TT.@new) | &{_spaceName != S.Def} @`#.`(TT, noMacro(@this)) | TT.@checked TT.LBrace TT.RBrace | TT.@unchecked TT.LBrace TT.RBrace | @`#.`(TT, noMacro(@default)) TT.Colon | TT.@using TT.LParen | (TT.@struct|@`#.`(TT, noMacro(@return))|TT.@while|TT.@try|TT.@switch|TT.@interface|TT.@enum|TT.@namespace|TT.@delegate|TT.@for|TT.@do|@`#.`(TT, noMacro(@throw))|TT.@fixed|TT.@goto|TT.@lock|TT.@case|TT.@class|@`#.`(TT, noMacro(@break))|TT.@event|@`#.`(TT, noMacro(@continue))|TT.@foreach) )))*
			 for (;;) {
				switch (LA0) {
				case TT.AttrKeyword:
					{
		...
		public LNode Stmt()
		{
			...
			LNode r;
			// Line 961: ( (UsingDirective | AssemblyOrModuleAttribute | EventDecl | DelegateDecl | SpaceDecl | EnumDecl) | (TraitDecl / AliasDecl / &(DataType ComplexNameDecl (TT.LambdaArrow|TT.Comma|TT.LParen|TT.Set|TT.Semicolon|TT.LBrace|TT.At|TT.Forward)) MethodOrPropertyOrVar / OperatorCast / Constructor / Destructor / BracedBlock / BlockCallStmt / ExprStatement) | CheckedOrUncheckedStmt | DoStmt | CaseStmt | GotoStmt | GotoCaseStmt | LabelStmt | ReturnBreakContinueThrow | WhileStmt | ForStmt | ForEachStmt | IfStmt | SwitchStmt | UsingStmt | LockStmt | FixedStmt | TryStmt | TT.Semicolon )
			 do {
				switch (LA0) {
				case TT.@using:
					{
		...
		LNode ExprStatement()
		{
			...
			// Line 1012: ((TT.@while|TT.@else|EOF|TT.@catch|TT.@finally) =>  | default TT.Semicolon)
			 switch (LA0) {
			case EOF:
			case TT.@catch:
			case TT.@else:
		...
		{
			TokenType la0;
			// Line 1021: greedy(~(TT.Semicolon|TT.LBrace|EOF))*
			 for (;;) {
				la0 = LA0;
		...
		LNode Constructor(int startIndex, RVList<LNode> attrs)
		{
			...
			LNode r;
			Token n;
			// Line 1328: ( &{_spaceName == LT($LI).Value} (TT.ContextualKeyword|TT.Id) &(TT.LParen TT.RParen (TT.Semicolon|TT.LBrace)) / &{_spaceName != S.Def} @`#.`(TT, noMacro(@this)) &(TT.LParen TT.RParen (TT.Semicolon|TT.LBrace)) / (TT.ContextualKeyword|@`#.`(TT, noMacro(@this))|TT.Id) &(TT.LParen TT.RParen TT.Colon) )
			 do {
				la0 = LA0;
				if (la0 == TT.ContextualKeyword || la0 == TT.Id) {
					if (_spaceName == LT(0).Value) {
		...
		{
			// Line 884: ( DataType (TT.AttrKeyword|TT.TypeKeyword|TT.ContextualKeyword|TT.Id) | (TT.AttrKeyword|TT.@new) | &{_spaceName != S.Def} @`#.`(TT, noMacro(@this)) | TT.@checked TT.LBrace TT.RBrace | TT.@unchecked TT.LBrace TT.RBrace | @`#.`(TT, noMacro(@default)) TT.Colon | TT.@using TT.LParen | (TT.@struct|@`#.`(TT, noMacro(@return))|TT.@while|TT.@try|TT.@switch|TT.@interface|TT.@enum|TT.@namespace|TT.@delegate|TT.@for|TT.@do|@`#.`(TT, noMacro(@throw))|TT.@fixed|TT.@goto|TT.@lock|TT.@case|TT.@class|@`#.`(TT, noMacro(@break))|TT.@event|@`#.`(TT, noMacro(@continue))|TT.@foreach) )
			 switch (LA0) {
			case TT.@operator:
			case TT.ContextualKeyword:
			case TT.Id:
