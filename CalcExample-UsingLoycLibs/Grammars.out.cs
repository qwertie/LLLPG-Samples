// Generated from Grammars.ecs by LeMP custom tool. LLLPG version: 1.3.0.0
...
namespace CalcExample
{
	...
	public partial class Calculator
	{
		...
		double Atom()
		{
			...
			// Line 253: ( TT.Id | TT.Num | TT.LParen ExprSequence TT.RParen )
			...
			if (la0 == TT.Id) {
				...
				#line 256 "Grammars.ecs"
				result = double.NaN;
				#line 256 "Grammars.ecs"
				Src.Error(0, "Expected identifer, number, or (parens)");
				#line default
			}
			// Line 259: greedy(TT.Exp Atom)*
			for (;;) {
				la0 = (TokenType) Src.LA0;
				if (la0 == TT.Exp) {
		...
		double AssignExpr()
		{
			TokenType la0, la1;
			double result = default(double);
			// Line 283: (TT.Id TT.Assign AssignExpr | AddExpr)
			la0 = (TokenType) Src.LA0;
			if (la0 == TT.Id) {
				la1 = (TokenType) Src.LA(1);
				if (la1 == TT.Assign) {
		...
		double ExprSequence()
		{
			...
			double result = default(double);
			result = AssignExpr();
			// Line 287: (TT.Semicolon AssignExpr)*
			for (;;) {
				la0 = (TokenType) Src.LA0;
				if (la0 == TT.Semicolon) {
					Src.Skip();
